...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True, to_tensor=True):
        ...
        # self.list_path = os.path.join(self.root_dir, split, f'{split}.txt')
...
class CDD_dataset(ImageDataset):

    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True):
        super(CDD_dataset, self).__init__(root_dir, img_size=img_size, split=split, is_train=is_train,
                                          to_tensor=to_tensor)
        self.label_transform = label_transform
        self.root_dir = os.path.join(self.root_dir, self.split)
...
class ImageDataset_CDD(data.Dataset):
    """VOCdataloder"""

    def __init__(self, root_dir, split='train', img_size=256, is_train=True, to_tensor=True):
        super(ImageDataset_CDD, self).__init__()
        self.root_dir = root_dir
        self.img_size = img_size
        self.split = split  # train | train_aug | val
        self.list_path = os.path.join(self.root_dir, split, f'{split}.txt')
...
class CDD(ImageDataset_CDD):

    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True):
        super(CDD, self).__init__(root_dir, img_size=img_size, split=split, is_train=is_train,
                                  to_tensor=to_tensor)
        self.label_transform = label_transform
        self.root_dir = os.path.join(self.root_dir, self.split)
